exports.run = (client, message, args) => {
    ...
    if(args === "List"){
        ...
        function generateEmbed(){
            let list_of_characters_by_rarity = list_of_characters.filter(character => 
                character.rarity === client.rarities.character[rarity_index]).map((character, index) => 
                `${index + 1}.) ${character.name}(${character.code_name}/${character.code_name_CN})`).join("\r\n");
            let generated_embed = {
    ...
    let selected_character_active_skills = Object.values(selected_character.skills.active).map(skill => {    
        ...
        if(skill.hasOwnProperty("multipliers")){
            Object.values(skill.multipliers).map((multiplier, index) => {
                skill.description = skill.description.replace(multipliers_placeholders[index], `${multiplier}/${multiplier * 2}/${multiplier * 3}(**${multiplier * 2}/${multiplier * 2 * 2}/${multiplier * 2 * 3}**)%`);
            })
        }
